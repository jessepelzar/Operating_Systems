...
LARGE_INTEGER LI_freq, starting_count, ending_count, count_diff;
void start_nanotime()
{	// first get the frequency value. QuadPart is a 64 bit integer (on a 64-bit PC)
	...
	QueryPerformanceCounter(&starting_count);
}
int get_nanodiff()
{
	double diff;
	// TBD: see if this allows for QuadPart to rollover to seconds
	QueryPerformanceCounter(&ending_count);
	// compute the count diff, then convert to nanosecs
	count_diff.QuadPart = ending_count.QuadPart - starting_count.QuadPart;
	diff = double(count_diff.QuadPart*10^6 / LI_freq.QuadPart);	// convert count to microseconds
	return int(diff);
}

#else
struct timespec myts_start,myts_end;
void start_nanotime()
{
clock_gettime(CLOCK_THREAD_CPUTIME_ID, &myts_start);
}
...
int get_nanodiff()
{
  ...
  struct timespec result;
	//start_time = myts.tv_nsec;
	clock_gettime(CLOCK_THREAD_CPUTIME_ID, &myts_end); // re-using the myts struct!!!
	//end_time = myts.tv_nsec;
	//return (int(end_time - start_time));  //nanosecs
	if ((myts_end.tv_nsec - myts_start.tv_nsec) < 0)
	{
		result.tv_sec = myts_end.tv_sec - myts_start.tv_sec - 1;
		result.tv_nsec = 10^9 + myts_end.tv_nsec - myts_start.tv_nsec;
	}
	else
	{
		result.tv_sec = myts_end.tv_sec - myts_start.tv_sec;
		result.tv_nsec = myts_end.tv_nsec - myts_start.tv_nsec;
	}
